...
const canaryPath = path.join(here, '../../bw-cfront/sb3-creator/examples/31-no-resistor-led/circuit.json');

describe('safety-lesson canary: 31-no-resistor-led', () => {
  if (!existsSync(canaryPath)) {
    it('SKIP: canary file not available', () => assert.ok(true));
    return;
  }
  ...
  it('DRC does not auto-fix the canary circuit', () => {
    ...
    const warnings = runDrc(c, c.board);
    assert.equal(c.parts.length, raw.parts.length,
      'DRC must not add or remove parts');
    assert.equal(c.wires.length, raw.wires.length,
      'DRC must not add or remove wires');
  });
