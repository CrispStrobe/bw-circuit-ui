...
describe('sweep-runner', () => {
  ...
  it('refusals are truthful: missing engine functions blame the host', () => {
    ...
    assert.ok(r2.reason.includes('runAcSweep'), r2.reason);
  });
  ...
  it('runBode forces a sine on the copy and hands logSpace freqs to runAcSweep', () => {
    let got = null;
    const engine = {
      BoardImpl: StubBoard,
    ...
    const r = runBode(engine, fakeLiveBoard(), { sourceId: 'vs', inNet: 'n_in', outNet: 'n_g', fFrom: 10, fTo: 1000 });
    assert.equal(r.ok, true);
