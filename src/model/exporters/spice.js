  ...
  for (const part of netlist.parts) {
    ...
    if (TWO_TERMINAL.has(card)) {
      ...
      if (value == null) {
        ...
        let metadata = '';
        if (card === 'R' && (part.params?.spiceTolerancePercent !== undefined
            || part.params?.spicePowerWatts !== undefined)) {
          const tol = part.params.spiceTolerancePercent;
          const pwr = part.params.spicePowerWatts;
          if ((tol !== undefined && (!Number.isFinite(tol) || tol < 0))
              || (pwr !== undefined && (!Number.isFinite(pwr) || pwr < 0))
              || part.params.spiceMetadataProvenance !== 'instance-card') {
            skipped.push(`${part.refdes} (resistor): invalid or unproven tolerance/power metadata`);
            lines.push(`* ${part.refdes} resistor — skipped (invalid or unproven tolerance/power metadata)`);
            continue;
          }
          if (tol !== undefined) metadata += ` tol=${formatSpiceValue(tol)}`;
          if (pwr !== undefined) metadata += ` pwr=${formatSpiceValue(pwr)}`;
        }
        lines.push(`${el} ${nodeFields} ${formatSpiceValue(value)}${metadata}`);
      }
    } else if (card === 'D') {
